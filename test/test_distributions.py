...
class TestDistributions(TestCase):
    def _gradcheck_log_prob(self, dist_ctor, ctor_params):
        ...
        s = distribution.sample().detach()
        # now that sample() creates a graph, we need to set samples as 
        # volatile for this test to pass
    ...
    def test_bernoulli(self):
        ...
        self.assertEqual(Bernoulli(r).sample(8).size(), (8, 1))
    ...
    def test_multinomial_1d(self):
        ...
        self.assertEqual(Multinomial(p).sample(1).size(), (1, 1))
    ...
    def test_multinomial_2d(self):
        ...
        self.assertEqual(Multinomial(p).sample(6).size(), (6, 2))
        self._gradcheck_log_prob(Multinomial, (p,))
    ...
    def test_normal(self):
        ...
        self.assertEqual(Normal(mean, std).sample(7).size(), (7, 5, 5))
        self.assertEqual(Normal(mean_1d, std_1d).sample(1).size(), (1, 1))
        self.assertEqual(Normal(mean_1d, std_1d).sample().size(), (1,))
        self.assertEqual(Normal(0.2, .6).sample(7).size(), (7, 1))
        self.assertEqual(Normal(-0.7, 50.0).sample(1).size(), (1, 1))
